...
- 文件管理：懒加载文件树、搜索、新建文件、与 Finder 双向复制/粘贴文件和文件夹、重命名、经确认后移入 macOS 废纸篓、在访达中显示、在目录中创建终端。
...
- SSH / SFTP：读取系统 `~/.ssh/config`、缓存最近连接、同站点多连接，并支持远端浏览、编辑、上传、下载、新建、重命名和安全删除。
...
- 原生文件拖放按指针位置路由：命中 SFTP 区域时上传到当前远端目录，其余区域交给当前选中的终端；终端路径经过 POSIX shell 转义，只插入文本，不自动提交命令。
...
### 7. SSH 主连接与 SFTP 文件生命周期

- SSH 连接仍运行在真实终端中。Berth 使用系统 `/usr/bin/ssh` 建立独立 `ControlMaster` 套接字，SFTP 复用该连接，因此密码、密钥口令和主机确认仍由终端处理。
- 站点来源为 `~/.ssh/config` 中明确的 Host 别名；`IdentityFile`、ProxyJump、端口、用户和通配规则继续由 OpenSSH 解释。
- 最近连接只在本地保存目标、显示名称、置顶状态和连接时间，不保存密码、密钥内容或临时套接字路径。
- SFTP 后端统一通过系统 `/usr/bin/sftp` 的批处理通道执行。目标、控制套接字和批处理路径均在 Rust 边界校验，路径中的换行和控制字符不会进入命令流。
- 本地文件树、SFTP 与 macOS 文件剪贴板共用一套复制/粘贴用例：支持 Finder 与 Berth 双向粘贴、本地互拷、远端互拷，以及本地与远端之间交叉粘贴。只有用户主动执行粘贴时才读取系统剪贴板，并通过 `changeCount` 判断 Berth 内的结构化来源是否仍然有效，不进行后台轮询。
- 粘贴目标优先使用选中的文件夹；选中文件时使用其所在目录。目标已有同名项目时生成“副本”“副本 2”等名称，不静默覆盖现有内容。
- 文件或文件夹移入废纸篓前必须确认；目标范围内存在未保存标签时拒绝移动。成功后关闭对应的已保存标签、清除失效剪贴板引用，并并行刷新文件树与 Git 状态。工作区根目录只允许从当前窗口移除，不提供整目录移入废纸篓入口。
- SFTP 跨连接复制通过权限为 `0700` 的系统临时目录中转，任务结束后立即递归清理。将远端项目复制到 Finder 时，本地缓存会保留到系统剪贴板被替换，以保证离开 Berth 后仍可粘贴；Berth 下次启动会清理超过 24 小时的异常遗留缓存。
- 远端文本最大 5 MB，保存前比较打开时的大小与修改时间；发现服务器端已变化时拒绝覆盖，并保留编辑器草稿。
- 未保存的远端标签与本地文件使用相同的关闭保护：只有保存成功才允许关闭。远端删除必须二次确认，目录只允许删除空目录。
- 图片、音频和视频先下载到有大小上限的系统临时缓存，再复用本地媒体预览；标签卸载或关闭时主动删除缓存。
- 上传、下载、保存和删除都提供持续的进行中状态。当前系统 SFTP 为单次批处理，进度条表示任务仍在执行，不伪造百分比。
